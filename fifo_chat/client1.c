...
int main() {
   ...
   if ((pid = fork()) < 0) {
      ...
      if ((fd = open(name_1, O_WRONLY)) < 0){
         printf("Can\'t open FIFO12 for writting\n");
	      exit(-1);
      }
      //считываем с консоли и записываем в фифо12
      while (1) {
         fgets(input_string, MAX_STRING_SIZE, stdin);
         size = write(fd, input_string, MAX_STRING_SIZE);
         if (size > MAX_STRING_SIZE){
            printf("Can\'t write all string to FIFO12\n");
            exit(-1);
         }
      }
      close(fd);
      ...
      //открываем для read
      if ((fd = open(name_2, O_RDONLY)) < 0) {
         printf("Can\'t open FIFO21 for reading\n");
	      exit(-1);
      }
      //читаем и печатаем
      while (1) {
