...
//цикл приема информации от клиента вынесен в отдельную нить для возможности подключения нескольких клиентов одновременно
//в эту нить передается структура необходимых данных
...
void* Thread(void* help) {
    ...
    while((helpp.n = read(helpp.newsockfd, helpp.line, 999)) > 0) {
            ...
            if((helpp.n = write(helpp.newsockfd, helpp.line, 
            strlen(helpp.line)+1)) < 0) {
                perror(NULL);
                close(helpp.sockfd);
